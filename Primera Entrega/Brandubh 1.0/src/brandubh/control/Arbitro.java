...
/**
 * El `Arbitro` controla y gestiona el desarrollo del juego Brandubh, aplicando
...
public class Arbitro {
	...
	private int numeroJugada; // Número de la jugada actual.
	private Color turnoActual; // El color del jugador que tiene el turno.
	...
	public Arbitro(Tablero tablero) {
	    this.tablero = tablero; // Asigna el tablero proporcionado al atributo de la clase.
	    ...
	    this.numeroJugada = 0; // Inicializa el número de jugada en cero.
	    trono = new Coordenada(tablero.consultarNumeroFilas() / 2, tablero.consultarNumeroColumnas() / 2); // Coordenada del trono en el tablero.
	}
	...
	public void colocarPiezas(TipoPieza[] tipo, int[][] coordenadas, Color turnoActual) {
	    if (tipo.length == coordenadas.length) { // Verifica que la cantidad de tipos y coordenadas sea la misma.
	        for (int i = 0; i < tipo.length; i++) { // Recorre los arreglos de tipos y coordenadas.
	            this.tablero.colocar(new Pieza(tipo[i]), new Coordenada(coordenadas[i][0], coordenadas[i][1])); // Coloca una pieza en las coordenadas proporcionadas.
	        }
	...
	public void colocarPiezasConfiguracionInicial() {
	    int filas = this.tablero.consultarNumeroFilas(); // Obtiene el número de filas del tablero.
	    int columnas = this.tablero.consultarNumeroColumnas(); // Obtiene el número de columnas del tablero.

	    for (int i = 0; i < filas; i++) { // Recorre las filas del tablero.
	        for (int j = 0; j < columnas; j++) { // Recorre las columnas del tablero.
	            if (i == mitad) { // Si está en la fila central del tablero:
	                if (Math.abs(j - mitad) < 4) { // Verifica si la columna está dentro de cierto rango de la fila central.
	                    if (Math.abs(j - mitad) > 1) { // Si la columna no está en la posición de las provincias o el trono:
	                        this.tablero.colocar(new Pieza(TipoPieza.ATACANTE), new Coordenada(i, j)); // Coloca un atacante en esa posición.
	                    } else if (j == mitad) { // Si está en la columna central:
	                        this.tablero.colocar(new Pieza(TipoPieza.REY), new Coordenada(i, j)); // Coloca al rey en esa posición.
	                    } else { // Si está en la posición de una provincia o el trono:
	                        this.tablero.colocar(new Pieza(TipoPieza.DEFENSOR), new Coordenada(i, j)); // Coloca un defensor en esa posición.
	                    }
	                }
	            }
	...
	public boolean esMovimientoLegal(Jugada jugada) {
	    ...
	    // Verifica si las celdas entre origen y destino están libres
	    return celdasLibres(jugada); // Los movimientos no pueden saltar por encima de otras piezas
	}


	/**
	 * Verifica si las celdas entre el origen y el destino de una jugada están libres.
	 * 
	 * @param jugada La jugada que se va a verificar.
	 * @return true si las celdas entre origen y destino están libres, de lo contrario, false.
	 */
	private boolean celdasLibres(Jugada jugada) {
	    // Obtiene el sentido de la jugada
	    Sentido s = jugada.consultarSentido();
	    ...
	    switch (s) {
	        case HORIZONTAL_E:
	            // Recorre las celdas en dirección este entre origen y destino
	            for (int i = origen.columna() + 1; i < destino.columna(); i++) {
	                // Verifica si la celda en la posición actual está vacía
	                if (!this.tablero.consultarCelda(new Coordenada(origen.fila(), i)).estaVacia())
	                    return false; // Si hay una pieza, retorna falso (celdas no libres)
	            }
	            break;
	        case HORIZONTAL_O:
	            // Recorre las celdas en dirección oeste entre origen y destino
	            for (int i = origen.columna() - 1; i > destino.columna(); i--) {
	                // Verifica si la celda en la posición actual está vacía
	                if (!this.tablero.consultarCelda(new Coordenada(origen.fila(), i)).estaVacia())
	                    return false; // Si hay una pieza, retorna falso (celdas no libres)
	            ...
	            break;
	        case VERTICAL_N:
	            // Recorre las celdas en dirección norte entre origen y destino
	            for (int i = origen.fila() - 1; i > destino.fila(); i--) {
	                // Verifica si la celda en la posición actual está vacía
	                if (!this.tablero.consultarCelda(new Coordenada(i, origen.columna())).estaVacia())
	                    return false; // Si hay una pieza, retorna falso (celdas no libres)        
	            }
	            break;
	        case VERTICAL_S:
	            // Recorre las celdas en dirección sur entre origen y destino
	            for (int i = origen.fila() + 1; i < destino.fila(); i++) {
	                // Verifica si la celda en la posición actual está vacía
	                if (!this.tablero.consultarCelda(new Coordenada(i, origen.columna())).estaVacia())
	                    return false; // Si hay una pieza, retorna falso (celdas no libres)
	    ...
	    return true; // Si no hay piezas entre el origen y destino, retorna verdadero (celdas libres)
	}
	 ...
	 */
	private int contarAtacantes(Celda[] contiguas) {
	 ...
	 */
	private int contarAtacantesVertical(Coordenada posicion) {
	 ...
	 */
	private int contarAtacantesHorizontal(Coordenada posicion) {
	 ...
	 */
	private int contarDefensoresVertical(Coordenada posicion) {
	    Celda[] contiguas = tablero.consultarCeldasContiguasEnVertical(posicion); // Obtiene las celdas contiguas en dirección vertical
	 ...
	 */
	private int contarDefensoresHorizontal(Coordenada posicion) {
	    Celda[] contiguas = tablero.consultarCeldasContiguasEnHorizontal(posicion); // Obtiene las celdas contiguas en dirección horizontal
	    return contarDefensores(contiguas); // Retorna el número total de defensores en las celdas contiguas
	}
	 ...
	 */
	private int contarDefensores(Celda[] contiguas) {
	...
	/**
	 * Verifica si el rey ha ganado la partida.
	 * 
	 * @return true si el rey ha ganado, de lo contrario false.
	 */
	public boolean haGanadoRey() {
		...
		for (Coordenada provincia : provincias) {
			...
			if (posicionRey().equals(provincia)) {
	...
	/**
	 * Realiza capturas de piezas después de mover una pieza en el tablero.
	 */
	 ...
	 */
	private void capturarDefensores() {
	 ...
	 * todas las direcciones adyacentes a la última pieza.
	 */
	private void capturarAtacantes() {
	 ...
	 */
	private void capturarDefensoresTodasDirecciones() {
	 ...
	 */
	private void capturarDefensorContraElTrono() {
	 ...
	 */
	private void capturarDefensorContraProvincia() {
	 ...
	 */
	private void capturarAtacanteContraProvincia() {
	 ...
	 */
	private void capturarAtacantesTodasDirecciones() {
	 ...
	 */
	private void capturarAtacanteContraElTrono() {
	 ...
	 */
	private Celda vecina(Coordenada c, Sentido d) {
	 ...
	 *          captura.
	 */
	private void capturarDefensor(Coordenada c, Sentido d) { // capturas contra provincias/trono
		...
		if (esDefensor(celda)) { // Comprueba si hay un defensor en la celda obtenida
			tablero.eliminarPieza(celda.consultarCoordenada());// Realiza la captura del defensor
		}
	}
	 ...
	 *          captura.
	 */
	private void capturarAtacante(Coordenada c, Sentido d) { // capturas contra provincias/trono
		// Obtiene la celda vecina en la dirección d desde la coordenada c
		Celda celda = vecina(ultimaPieza, d);
	 ...
	 *           atacantes.
	 */
	private void capturarDefensorRodeado(Coordenada up, Sentido d) {
	 ...
	 *           defensores.
	 */
	private void capturarAtacanteRodeado(Coordenada up, Sentido d) {
